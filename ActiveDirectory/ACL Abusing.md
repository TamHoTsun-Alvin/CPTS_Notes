...
- OutSyl for GenericWrite (fluffy): If a certain group or user have GenericWrite over a certain member, what we can do is to use certipy-ad to add shadow credentials to that specific account and use it to retrieve the RC4 password hash and NT hash - This won't grant us full access but now we can have winrm or rdp session with the respective user and perform action that can be done with PTH.
